...
class BertForDeepSeaClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    Examples::
        ...
        from transformers import BertTokenizer, BertForDeepSeaClassification
        import torch

        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = BertForDeepSeaClassification.from_pretrained('bert-base-uncased')
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = BCELoss()
                sigmoid = nn.Sigmoid()
                loss = loss_fct(sigmoid(logits), labels.float())
                # loss_fct = CrossEntropyLoss()
                # loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            outputs = (loss,) + outputs
